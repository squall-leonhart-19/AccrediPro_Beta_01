#!/usr/bin/env node
/**
 * Export all DFY purchases from the last 7 days (all prices) to CSV
 *
 * Usage: node scripts/export-dfy-7days.cjs
 *
 * Connects to the database via Prisma, finds all DFYPurchase records
 * from the last 7 days, includes user + product info, checks
...
async function main() {
  ...
  try {
    ...
    // 2. Fetch all DFYPurchase records from the last 7 days (all prices)
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

    console.log(`Querying purchases since: ${sevenDaysAgo.toISOString().split('T')[0]}`);
    ...
    console.log(`Found ${purchases.length} purchases in the last 7 days`);
    ...
    const outputPath = path.resolve(__dirname, 'dfy-last-7-days.csv');
    ...
    const priceCounts = {};
    for (const p of purchases) {
      const price = parseFloat(p.purchasePrice.toString()).toFixed(2);
      priceCounts[price] = (priceCounts[price] || 0) + 1;
    }
    console.log('\nPrice Breakdown:');
    for (const [price, count] of Object.entries(priceCounts).sort((a, b) => parseFloat(b[0]) - parseFloat(a[0]))) {
