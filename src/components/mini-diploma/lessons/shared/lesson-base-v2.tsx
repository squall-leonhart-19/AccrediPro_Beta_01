...
import {
    ...
    Award, Play, Pause, DollarSign,
} from "lucide-react";
import { useMiniDiplomaTracking } from "@/hooks/use-mini-diploma-tracking";

const SARAH_AVATAR = "/coaches/sarah-coach.webp";

// Extended message types for interactive elements
...
export interface Message {
    ...
    renderComponent?: (props: { firstName: string; onComplete: () => void }) => ReactNode;
}

interface LessonBaseV2Props {
    lessonNumber: number;
    lessonTitle: string;
...
export function LessonBaseV2({
    ...
    // Tracking
    const { trackLessonStarted, trackLessonCompleted } = useMiniDiplomaTracking();

    const calculateTypingDelay = (content: string): number => {
        const baseDelay = Math.ceil(content.length / 50) * 1000;
        const randomFactor = Math.random() * 400 - 200;
        return Math.max(1000, Math.min(baseDelay + randomFactor, 3000));
    };
    ...
    // Track lesson start
    useEffect(() => {
        if (!hasTrackedStart.current && !isCompleted) {
            hasTrackedStart.current = true;
            lessonStartTime.current = Date.now();
            trackLessonStarted(lessonNumber, lessonTitle);
        }
    }, [lessonNumber, lessonTitle, isCompleted, trackLessonStarted]);

    // Interactive message types that pause progression
    const interactiveTypes: MessageType[] = [
        'user-choice',
        'self-assessment',
        'case-study',
        'knowledge-check',
    ...
    useEffect(() => {
        if (waitingForInteraction || currentMessageIndex >= messages.length) return;

        const currentMsg = messages[currentMessageIndex];

        if (interactiveTypes.includes(currentMsg.type)) {
            setIsTyping(false);
        ...
        if (currentMsg.type === 'pre-recorded-audio') {
            setIsTyping(true);
    ...
    const handleComplete = async () => {
        // Track lesson completion with duration
        const durationSeconds = Math.round((Date.now() - lessonStartTime.current) / 1000);
        trackLessonCompleted(lessonNumber, lessonTitle, durationSeconds, currentScore);
                    ...
                    {currentScore > 0 && (
                        <div className="bg-indigo-50 rounded-xl p-3 mb-4 text-center">
                            <p className="text-sm text-indigo-600">
                                Practitioner Score: <span className="font-bold">{currentScore}</span> points
                            </p>
                        </div>
                    )}
    ...
    const renderMessage = (msg: Message) => {
        switch (msg.type) {
            ...
            case 'practitioner-score':
                // Render custom component if provided
                if (msg.renderComponent) {
                    return msg.renderComponent({
                        firstName,
                        onComplete: () => handleInteractionComplete(msg.componentProps?.points || 0)
                    });
                }
                return null;
            ...
            <div className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-burgundy-100 px-4 py-3 shadow-sm">
                <div className="max-w-2xl mx-auto">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1 text-center">{lessonTitle}</p>
                    </div>
                </div>
            </div>
