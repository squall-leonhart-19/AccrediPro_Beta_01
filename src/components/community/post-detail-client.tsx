    ...
    <div className="min-h-screen bg-gray-50 font-sans pb-20">
      ...
      <div className="px-4 sm:px-6 lg:px-12 xl:px-20 py-6 space-y-4">
        ...
        <main className="max-w-4xl mx-auto space-y-6">
          {/* Post Card with School Branding */}
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {post.author.role === "MENTOR" && (
                      <span className="bg-gradient-to-r from-burgundy-600 to-burgundy-700 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide shadow-sm">
                        ✓ Mentor
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-gray-500">
                    {new Date(post.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} • <span className="inline-flex items-center gap-1 bg-burgundy-50 text-burgundy-700 px-2 py-0.5 rounded-full font-medium">{post.category}</span>
                  </p>
                </div>
              </div>
              {isAdminOrMentor ? (
            ...
            {/* Reaction Counts - Enhanced Pills */}
            <div className="px-5 py-3 flex items-center gap-2 flex-wrap border-t border-gray-50">
              {Object.entries(postReactions)
                ...
                .map(([emoji, count]) => count > 0 && (
                  <span
                    key={emoji}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold bg-gray-50 text-gray-700 border border-gray-100 hover:bg-gray-100 transition-colors cursor-pointer"
                  >
                    <span className="text-base">{emoji}</span> {count}
                  </span>
                ))}            </div>

            {/* Interaction Bar - Enhanced */}
            <div className="px-5 py-4 border-t border-gray-100 grid grid-cols-2 gap-3 relative bg-gray-50/50">
          ...
          {/* Comment Input - Enhanced Card */}
          <div className="p-5 bg-white rounded-2xl shadow-lg border border-gray-100 flex gap-4 items-start">
            <Avatar className="w-10 h-10 ring-2 ring-burgundy-50">
              <AvatarImage src={currentUserImage || undefined} />
              <AvatarFallback className="bg-burgundy-100 text-burgundy-700 font-bold">{currentUserFirstName?.[0]}</AvatarFallback>
            </Avatar>
            <div className="flex-1">
              <textarea
                className="w-full bg-gray-50 rounded-xl border border-gray-200 resize-none p-4 text-sm focus:ring-2 focus:ring-burgundy-100 focus:border-burgundy-300 focus:bg-white transition-all min-h-[60px]"
              ...
              <div className="flex justify-end mt-3">
                <Button
                  ...
                  className="bg-gradient-to-r from-burgundy-600 to-burgundy-700 hover:from-burgundy-700 hover:to-burgundy-800 text-white font-semibold px-6 rounded-xl shadow-sm"
                >
              ...
              {comments.map((comment: any) => (
                <div key={comment.id} className="p-5">
                  <div className="flex gap-3">
                    <Avatar className="w-10 h-10 shrink-0 ring-1 ring-gray-100">
                      <AvatarImage src={comment.author.avatar} />
                      <AvatarFallback className="bg-burgundy-50 text-burgundy-700 text-sm font-bold">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          {comment.author.role === "MENTOR" && (
                            <span className="text-[10px] bg-burgundy-100 text-burgundy-700 font-bold px-1.5 py-0.5 rounded-full">✓ Mentor</span>
                          )}
                        </div>
                        {/* Admin Delete Button */}
                        {isAdminOrMentor && (
