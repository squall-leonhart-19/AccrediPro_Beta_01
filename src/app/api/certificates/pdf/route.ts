...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate the certificate HTML based on type
    let htmlCode: string;
    if (type === "mini-diploma") {
      htmlCode = generateMiniDiplomaCertificateHTML({
        studentName: formattedName,
        diplomaTitle: diplomaTitle || "Functional Medicine Mini Diploma",
        completedDate: formattedDate,
        certificateId,
      });
    } else {
      htmlCode = generateCertificateHTML({
        studentName: formattedName,
    ...
    const filename = type === "mini-diploma"
      ? `${(diplomaTitle || "Mini-Diploma").replace(/\s+/g, "-")}-Certificate.pdf`
      : `${cleanModuleTitle.replace(/\s+/g, "-")}-Certificate.pdf`;
...
interface MiniDiplomaCertificateData {
  studentName: string;
  diplomaTitle: string;
  completedDate: string;
  certificateId: string;
...
function generateMiniDiplomaCertificateHTML(data: MiniDiplomaCertificateData): string {
  const { studentName, diplomaTitle, completedDate, certificateId } = data;
