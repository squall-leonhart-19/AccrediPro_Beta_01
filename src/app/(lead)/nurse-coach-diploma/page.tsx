...
const LESSONS = [
    { id: 1, title: "From Bedside to Business", module: 1 },
    { id: 2, title: "The Coaching Mindset", module: 1 },
    { id: 3, title: "Burnout Recovery", module: 1 },
    { id: 4, title: "Core Coaching Skills", module: 2 },
    { id: 5, title: "Health Behavior Change", module: 2 },
    { id: 6, title: "Building Your Practice", module: 2 },
    { id: 7, title: "Marketing for Nurses", module: 3 },
    { id: 8, title: "Legal & Ethical Considerations", module: 3 },
...
async function getLeadProgress(userId: string) {
    const [user, enrollment, leadOnboarding, completionTags] = await Promise.all([
        ...
        prisma.enrollment.findFirst({
            where: { userId, course: { slug: "nurse-coach-mini-diploma" } },
        }),
        prisma.leadOnboarding.findUnique({ where: { userId }, select: { watchedVideo: true, completedQuestions: true } }).catch(() => null),
        prisma.userTag.findMany({ where: { userId, tag: { startsWith: "nurse-coach-lesson-complete:" } } }),
    ]);

    if (!enrollment) return null;
    const completedLessons = new Set(completionTags.map((t) => parseInt(t.tag.replace("nurse-coach-lesson-complete:", ""))));
    return { user, leadOnboarding, completedLessons: Array.from(completedLessons) };
}

export default async function NurseCoachDiplomaPage() {
