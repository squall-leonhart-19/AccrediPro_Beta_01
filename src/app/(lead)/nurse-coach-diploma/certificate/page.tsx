...
async function getCertificateData(userId: string) {
    const [user, completionTags] = await Promise.all([
        ...
        prisma.userTag.findMany({
            where: {
                userId,
                tag: { startsWith: "nurse-coach-lesson-complete:" },
            },
        }),
    ]);
...
export default async function CertificatePage() {
    ...
    const certificateId = `NC-${session.user.id.slice(0, 8).toUpperCase()}`;

    if (!isComplete) {
        redirect("/nurse-coach-diploma");
    }
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 md:p-8">
            <div className="max-w-3xl mx-auto">
                {/* Back Button */}
                <Link href="/nurse-coach-diploma/complete" className="inline-flex items-center text-gray-600 hover:text-burgundy-600 mb-6">
