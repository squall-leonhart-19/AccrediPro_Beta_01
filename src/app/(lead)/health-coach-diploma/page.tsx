...
const LESSONS = [
    { id: 1, title: "What is Health Coaching?", module: 1 },
    { id: 2, title: "Core Coaching Competencies", module: 1 },
    { id: 3, title: "The Wellness Wheel", module: 1 },
    { id: 4, title: "Goal Setting & Action Plans", module: 2 },
    { id: 5, title: "Motivational Interviewing", module: 2 },
    { id: 6, title: "Nutrition Foundations", module: 2 },
    { id: 7, title: "Stress & Lifestyle", module: 3 },
...
async function getLeadProgress(userId: string) {
    const [user, enrollment, leadOnboarding, completionTags] = await Promise.all([
        ...
        prisma.enrollment.findFirst({
            where: { userId, course: { slug: "health-coach-mini-diploma" } },
        }),
        prisma.leadOnboarding.findUnique({ where: { userId }, select: { watchedVideo: true, completedQuestions: true } }).catch(() => null),
        prisma.userTag.findMany({ where: { userId, tag: { startsWith: "health-coach-lesson-complete:" } } }),
    ]);

    if (!enrollment) return null;
    const completedLessons = new Set(completionTags.map((t) => parseInt(t.tag.replace("health-coach-lesson-complete:", ""))));
    return { user, leadOnboarding, completedLessons: Array.from(completedLessons) };
}

export default async function HealthCoachDiplomaPage() {
