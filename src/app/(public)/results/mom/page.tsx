...
function MomResultsInner() {
  ...
  const SCHOLARSHIP_REASONS = [
    ...
    { reason: "Founder's Circle", why: "Dr. Sarah Mitchell's personal mission is making FM accessible to busy moms. She funds 3 scholarships per month.", icon: "💜" },
  ];
    ...
    <div className="min-h-screen" style={{ background: `linear-gradient(180deg, ${B.cream} 0%, #f5f0e8 30%, ${B.cream} 100%)` }}>
      ...
      <div className="max-w-5xl lg:max-w-[1200px] mx-auto px-3 sm:px-4 lg:px-8 py-6 sm:py-8 md:py-14 space-y-6 sm:space-y-8">
        ...
        <Section>
          ...
          <SectionInner className="text-center space-y-5 sm:space-y-6">
            ...
            <div className="inline-flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-widest" style={{ backgroundColor: `${B.burgundy}10`, color: B.burgundy }}>
              <Award className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> For Stay-at-Home Moms
            </div>

            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-[2.75rem] font-extrabold leading-[1.15]" style={{ color: B.burgundyDark }}>
              {firstName}, Your New At-Home Business Could Be Worth {income.label}
              <br />
        ...
        <Section bg={`${B.gold}08`}>
          <div className="px-4 sm:px-6 py-3" style={{ background: B.goldMetallic }}>
            <span className="text-xs sm:text-sm font-bold flex items-center gap-2" style={{ color: B.burgundyDark }}>
              <Gift className="w-4 h-4" /> Mom-Preneur Track Bonuses (Included Free)
            </span>
          </div>
        ...
        <Section>
          <SectionInner className="text-center space-y-4 sm:space-y-5">
            ...
            <p className="text-xs text-gray-500 italic">We can offer this because 97% of mom entrepreneurs who start — stay. The training speaks for itself.</p>
          </SectionInner>
        </Section>
          ...
          <SectionInner className="space-y-4">
            ...
            <div className="space-y-3">
              {[
                ...
                { objection: '"Do I need a medical degree or specific license?"', crush: "No medical degree required. The DEPTH certification is designed for mom entrepreneurs AND career changers. Nurses, PAs, mom entrepreneurs, nutritionists — even women with zero clinical background have graduated and built practices earning $8K+/month." },
