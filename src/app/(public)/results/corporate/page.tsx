...
// ─── Main ──────────────────────────────────────────────────────────
function CorporateResultsInner() {
  ...
  const role = sp.get("role") || "corporate";
  ...
  const SCHOLARSHIP_REASONS = [
    ...
    { reason: "Founder's Circle", why: "Dr. Sarah Mitchell's personal mission is making FM accessible to women escaping corporate. She funds 3 scholarships per month.", icon: "💜" },
  ];
    ...
    <div className="min-h-screen" style={{ background: `linear-gradient(180deg, ${B.cream} 0%, #f5f0e8 30%, ${B.cream} 100%)` }}>
      ...
      <div className="max-w-5xl lg:max-w-[1200px] mx-auto px-3 sm:px-4 lg:px-8 py-6 sm:py-8 md:py-14 space-y-6 sm:space-y-8">
        ...
        <Section>
          ...
          <SectionInner className="text-center space-y-5 sm:space-y-6">
            ...
            <div className="inline-flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-widest" style={{ backgroundColor: `${B.burgundy}10`, color: B.burgundy }}>
              <Award className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> For Corporate Professionals
            </div>

            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-[2.75rem] font-extrabold leading-[1.15]" style={{ color: B.burgundyDark }}>
              {firstName}, Your New Practice Could Be Worth {income.label}
              <br />
              ...
              <p className="text-xs sm:text-sm text-gray-500 max-w-lg mx-auto">
                {currentIncome === "0" || currentIncome === "under-2k"
                  ...
                  : currentIncome === "over-5k"
                    ...
                    : `They were in your exact shoes — corporate professionals earning ${curIncome.label}/month, wondering if there was something more. There was.`
                }
              </p>
            </div>
        ...
        <Section bg={`${B.gold}08`}>
          <div className="px-4 sm:px-6 py-3" style={{ background: B.goldMetallic }}>
            <span className="text-xs sm:text-sm font-bold flex items-center gap-2" style={{ color: B.burgundyDark }}>
              <Gift className="w-4 h-4" /> Corporate Escape Track Bonuses (Included Free)
            </span>
          </div>
        ...
        <Section>
          <SectionInner className="text-center space-y-4 sm:space-y-5">
            ...
            <p className="text-xs text-gray-500 italic">We can offer this because 97% of corporate professionals who start — stay. The training speaks for itself.</p>
          </SectionInner>
        </Section>
          ...
          <SectionInner className="space-y-4">
            ...
            <div className="space-y-3">
              {[
                ...
                { objection: '"Do I need a medical degree or specific license?"', crush: "No medical degree required. The DEPTH certification is designed for corporate professionals AND career changers. Nurses, PAs, corporate professionals, nutritionists — even women with zero clinical background have graduated and built practices earning $8K+/month." },
        ...
        <Section>
          <div className="px-4 sm:px-6 py-3" style={{ background: startTimeline === "this-week" ? B.burgundy : B.goldMetallic }}>
            <span className="text-xs sm:text-sm font-bold" style={{ color: startTimeline === "this-week" ? "white" : B.burgundyDark }}>
              <AlertTriangle className="w-3.5 h-3.5 sm:w-4 sm:h-4 inline mr-1.5 -mt-0.5" /> {startTimeline === "this-week" ? "URGENT — Your Cohort Starts This Week" : "Limited Availability — Corporate Escape Track Cohort"}
            </span>
          </div>
          <SectionInner className="text-center space-y-5 sm:space-y-6">
            ...
            <p className="text-sm sm:text-base text-gray-600 max-w-lg mx-auto">
              {vision === "leave-job" && `You said you want to leave your 9-to-5. This is the fastest path. ${urgencySpots} spots left in the Corporate Escape Track.`}
      ...
      <ScholarshipChat
        ...
        page="corporate-results"
      />
    </div>
  );
}

export default function CorporateResultsPage() {
  return (
    <Suspense fallback={
