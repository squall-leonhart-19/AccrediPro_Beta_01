/**
 * Women's Health - Dense Sarah DM Nurture Content v3.0 (10/10)
 * 
...
export const TIME_BASED_DMS: SarahDM[] = [
...
I'm Sarah, your guide through the Women's Health Mini Diploma. I'll be here every step of the way.
...
export const WH_DM_SCHEDULE = TIME_BASED_DMS.map(dm => ({
    day: dm.day!,
    triggerId: dm.id,
})).sort((a, b) => a.day - b.day);

export function getDMContent(id: string): SarahDM | undefined {
    return [...TIME_BASED_DMS, ...LESSON_COMPLETION_DMS, ...BEHAVIORAL_DMS].find(dm => dm.id === id);
}
